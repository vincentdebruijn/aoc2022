...
const MULTIPLY_CONSTANT = 1_000_000;
...
const satellites =
    space.flatMap((row, y) => row.map((el, x) => [el === '#', x, y]))
         ...
         .map(([_, x, y]) => ({x, y}))
         .map(({x, y}) => ({
              x: x + insertIndexOfSortedList(emptyColumnIndices, x) * (MULTIPLY_CONSTANT - 1),
              y: y + insertIndexOfSortedList(emptyRowIndices, y) * (MULTIPLY_CONSTANT - 1)
          }));
